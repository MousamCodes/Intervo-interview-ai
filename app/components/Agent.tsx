...
const Agent = ({userName}: AgentProps) => {
  const status = CallStatus.ENDED; // Placeholder for call status
  const isSpeaking = true; // Placeholder for speaking stat
 const [messages] = useState([
  'what is your name?',
  'My name is Intervo, your virtual interview assistant.'
]);
        ...
        <button className="relative btn-call">
          <span className={cn('absolute animate-ping rounded-full opacity-75'),
           status!== 'connecting' &'hidden'}
            ...
            <span>
              {status == 'inactive'|| status === 'ended' ?'CALL':'. . .'}
            </span>
